...
var page = 0
var page_count = 20
var count_page =  Math.ceil(data_list.length/page_count);
var reason_respone = ''
if (data_list.lenght != 0){
    drow_main()
}
function date_2_str(date){
    let end_date_str = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate();
    return end_date_str
}
function com_date(date,delta_date){
    let start_date = date;
    start_date.setDate(start_date.getDate() - delta_date);
    let start_date_str = date_2_str(start_date);
    return start_date_str
}
function init_html(option){
    let today_str = com_date(date,delta_date = 0)
    if (option == "monitor"){
        let end_date_str = date_2_str(date)
        var param=document.getElementsByClassName('param')[0]
<!--        param.innerHTML=`<td>监控数据日期:</td><input type="text" name="monitor_input" value="${today_str}">-->
<!--                        <input type="submit" value="查询" id="monitor_btn">`-->
        param.innerHTML=`
            <form method="POST"  style="padding-left:100px;padding-right:100px;top:0px" >
            <td>监控数据日期:</td><input type="text" name="monitor_input" value="${today_str}">
            <input type="submit" value="查询" id="monitor_btn"></form>`;
            var reason_respone = 'monitor'
    }
    else if (option == "remen_xiaoboxin_B"){
        let end_date_str = date_2_str(date)
        let start_date_str = com_date(date,delta_date = 180)
        var param=document.getElementsByClassName('param')[0]
        param.innerHTML=`<form method="POST"  style="padding-left:100px;padding-right:100px;top:0px" >
                        <td>目标日期:</td><input type="text" name="remen_xiaoboxin_B_today_input" value="${today_str}">
                        <td>起始分数:</td><input type="text" name="remen_xiaoboxin_B_input_grade_s" value="0">
                        <td>终止分数:</td><input type="text" name="remen_xiaoboxin_B_input_grade_e" value="100000">
                        <td>开始日期:</td><input type="text" name="remen_xiaoboxin_B_input_date_s" value="${start_date_str}">
                        <td>终止日期:</td><input type="text" name="remen_xiaoboxin_B_input_date_e" value="${end_date_str}">
                        <input type="submit" value="查询" id="xiaoboxin_btn"></form>`;
    }
    else if (option == "zhuang"){
        let end_date_str = date_2_str(date)
        let start_date_str = com_date(date,delta_date = 360)
        var param=document.getElementsByClassName('param')[0]
        var reason_respone = 'zhuang'
        param.innerHTML=`<form method="POST"  style="padding-left:100px;padding-right:100px;top:0px" >
                        <td>起始分数:</td><input type="text" name="zhuang_input_grade_s" value="1000">
                        <td>终止分数:</td><input type="text" name="zhuang_input_grade_e" value="10000">
                        <td>开始日期:</td><input type="text" name="zhuang_input_date_s" value="${start_date_str}">
                        <td>终止日期:</td><input type="text" name="zhuang_input_date_e" value="${end_date_str}">
                        <input type="submit" value="查询" id="zhuang_btn"></form>`;
    }
    else if (option == "remen_5"){
        let end_date_str = date_2_str(date)
        let start_date_str = com_date(date,delta_date = 180)
        var param=document.getElementsByClassName('param')[0]
        param.innerHTML=`<form method="POST"  style="padding-left:100px;padding-right:100px;top:0px" >
                        <td>目标日期:</td><input type="text" name="remen_5_today_input" value="${today_str}">
                        <td>起始分数:</td><input type="text" name="remen_5_grade_s" value="10000">
                        <td>终止分数:</td><input type="text" name="remen_5_grade_e" value="1000000">
                        <td>开始日期:</td><input type="text" name="remen_5_date_s" value="${start_date_str}">
                        <td>终止日期:</td><input type="text" name="remen_5_date_e" value="${end_date_str}">
                        <input type="submit" value="查询" id="remen_5_btn"></form>`;
    }
    else if (option == "user_define"){
        var param=document.getElementsByClassName('param')[0]
        param.innerHTML=`<form method="POST"  style="padding-left:100px;padding-right:100px;top:0px" >
                        <td>sql:</td><input type="text" name="user_define" value="">
                        <input type="submit" value="查询" id="user_define_btn"></form>`;
    }
}
...
<!--var data = splitData([['2015-10-16',18.4,18.58,18.33,18.79,67.00,1,0.04,0.11,0.09],['2015-10-19',18.56,18.25,18.19,18.56,55.00,0,-0.00,0.08,0.09]])-->
...
function go_page(page){
    list_start = page * page_count
    if (data_list.lengh <= page*page_count){
        end_no = data_list.lengh%page_count
    }
    else{
        end_no = page_count
    }
...
<!--        mycharts = undefined;-->
<!--        }-->
    for (let i=0;i<end_no;i++){
        newhtml = newhtml + `<div class="main" style="width: 1200px; height: 500px;padding-left:255px;"></div>` +
         `<div class="fill" style="width: 1200px; height: 80px;"></div>`
        }
        <!--newhtml =`<div class="main" style="width: 1200px; height: 500px;">test1</div>`-->
    var content=document.getElementsByClassName('content')[0]
    content.innerHTML=newhtml
    for (let j=list_start;j<(list_start+end_no);j++){
        i = j%page_count;
        console.log('len_list:',j,data_list.length)
        let data_single = data_list[j];
<!--        console.log(`len_single:${data_single}`)-->
        let table = data_single[0]
        let info = data_single[2]
        let stock_data = data_single[3]
        let code = data_single[1]
        ...
        function del_div(j){
            var e = document.getElementsByClassName('main')[j];
            document.getElementsByClassName('content')[0].removeChild(e);
            var f = document.getElementsByClassName('fill')[j];
            document.getElementsByClassName('content')[0].removeChild(f);
        }
        <!--echarts.init(content.getElementsByClassName('main')[0]).setOption(option);-->
        <!--echarts.init(content.getElementsByClassName('main')[1]).setOption(option);-->

        var mycharts = echarts.init(content.getElementsByClassName('main')[i])
        mycharts.setOption(option);
        <!--content.getElementsByClassName('main')[i].innerHTML=`<button type="button" class="button btn-light" id="add-btn" onclick=`del_div(${j})`>删除${i}</button>`-->
        var del_but = document.createElement("div");
        <!--del_but.type = 'button';-->
        <!--del_but.value = '删除';-->
<!--        del_but.innerHTML=`<button type="button" class="button btn-light" id="add-btn" onclick="del_div(${j})">删除${i}</button>`-->
<!--        del_but.innerHTML=`<div style="padding-left:800px;padding-right:100px;top:0px"><input type="text" name="reason+${data_single[0]}" value=""><input type="submit" value="删除" id="add-btn" onSubmit="del_div(0)"></div>`;-->
        del_but.innerHTML=`<form method="POST"  style="padding-left:800px;padding-right:100px;top:0px" target="iframename">
            <input type="text" name="reason${code}${table}" value=""><input type="submit" value="删除" id="add-btn"></form><iframe name="iframename" style="display:none;"></iframe>`;
        document.getElementsByClassName('main')[i].appendChild(del_but);
    }
    page_button(page);
}

function page_fun(flag){
    if (flag == 'next'){
        page ++;
        go_page(page)
    }
...
function page_button(page){
    let page_str = page +1
    page_html = `<div style="padding-left:600px;top:0px"><td>第${page_str}页，共 ${count_page} 页</td>`
    if (page == 0){
        page_html += `<input type="submit" value="下一页" id="add-btn" onclick="page_fun('next')"></div>`;
    }
    else if(page == count_page-1){
        page_html += `<input type="submit" value="上一页" id="add-btn" onclick="page_fun('pre')"></div>`;
    }
    else {
        page_html += `<input type="submit" value="上一页" id="add-btn" onclick="page_fun('pre')">
        <input type="submit" value="下一页" id="add-btn" onclick="page_fun('next')"></div>`;
    }
    let page_but = document.createElement("div");
    page_but.innerHTML = page_html
    <!--page_but.innerHTML=`<div style="padding-left:1000px;top:0px"><input type="submit" value="下一页" id="add-btn" onclick="go_page(${start_no+10})"></div>`;-->
<!--    document.getElementsByClassName('page')[0].appendChild(page_but);-->
